#
#   Template for 15-110 Homework #3
#
#   Problem #2a: letterCount
#
#
#   WRITTEN BY (NAME & ANDREW ID):
#
#   15-110 section:

# Implement the function  letterCount(string, letter) after this line

def letterCount(string, letter):
    count = 0
    for ch in string:
        if ch == letter:
            count += 1
    return count
    
        



# These assertions are here for your benefit.  They are not exhaustive:
# the CA's will use more tests when grading your final submissions.
# Feel free to add/remove tests here, just be careful!

assert(letterCount("abc-123-abc-ABC", "b") == 2)
assert(letterCount("This is interesting!", "z") == 0)

print("Passed all tests!")

