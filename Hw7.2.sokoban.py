...
#
#
#   WRITTEN BY (NAME & ANDREW ID):Moosuk    moosukk
#
#   15-110 section:N
...
"." is the solid black background. The filler between the walls and the edge of canvas.
"p" is the location of the player
b) What does it mean if isGoal[row][col] is True?
It means that the ball is on a goal spot and it will turn gold.
c) What does the findPlayer function do?
It finds the location of "p" to help the function movePlayer so that all movePlayer
has to do is add or subtract one to the row or col located by findPlayer.
d) Briefly explain why the following moves the player to the left:
        movePlayer(canvas, 0, -1)
    When dcol is -1, you subtract 1 from the col location of "p". It moves "p"
    one col to the left so graphically, the player moves left.
...
def undoFunction():
    init(canvas)
    for x in len(undoListRows):
        movePlayer(canvas, undoListRows, undoListCols)
        redrawAll(canvas)
...
def init(canvas):
    ...
    canvas.data["undoListRows"] = undoListRows
    canvas.data["undoListCols"] = undoListCols
